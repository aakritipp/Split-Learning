...
from lora import apply_lora_to_opt, iter_lora_parameters
...
def _refresh_eval_prefixes(full_model, server_model, trainer):
    ...
    try:
        ...
        if isinstance(resp, dict) and resp.get("type") == "client_kv_state":
            ...
            return True
        if args.tuning != "lora":
            model.enable_prefix_eval(False)
            return True
        ...
        for batch_idx, batch in enumerate(tqdm(test_loader, desc="SQUAD Evaluation")):
            try:
                ...
                h_cut_live, pkg = _server_forward_to_cut_payload(
                    server_model,                     # your live server wrapper (prefix or LoRA or none)
                ...
                assert isinstance(resp, dict) and resp.get("type") == "eval_stats", f"Bad eval resp: {type(resp)}"

                # server-side aggregation
                loss = float(resp["loss"])
                answer_acc = float(resp.get("answer_acc", 0.0))
                f1  = float(resp.get("f1", 0.0))
                em  = float(resp.get("em", 0.0))
