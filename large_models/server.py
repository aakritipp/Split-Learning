...
# from prefix_kv import PrefixKV, load_grad_state_into, merge_past_key_values
...
#         # If we've enabled prefix-aware eval and have both halves, prefill cache
#         if self._use_prefix_eval and (self._server_kv_live is not None) and (self._client_kv_eval is not None):
...
#             # infer prefix length P from first available layer
#             past_len = 0
...
#         # Fallback if no prefixes are loaded
...
#                 outputs = model(input_ids, attention_mask, labels)  # unchanged after this
...
#         try:
#             trainer.send_data({'type': 'training_complete'})
#             print("\nTraining completed successfully! (client notified)")
#         except Exception as e:
#             print(f"\n⚠️ Could not notify client (likely already closed): {e}")
