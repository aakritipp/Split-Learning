...
class PrefixTuning:
    ...
    def __init__(self, model, num_prefix, reparam=True, embed_dim=None, mid_dim=None, float16=False, init_by_real_act=False):
        ...
        # Hidden dimension and heads in a HF-like config (GPT-2 split config
        # exposes these aliases too).
        ...
        if mtype == "opt":
            ...
            first_layer_name = "layers.0"
            layer_name = "layers."
        elif mtype in ["gpt2", "gpt2_split"]:
            # Our custom GPT-2 and split GPT-2 use blocks under `.h`
            attention_name = "attn"
            first_layer_name = "h.0"
            layer_name = "h."
        else:
            raise NotImplementedError(f"PrefixTuning does not support model_type={mtype}")

        if init_by_real_act:
            # Initialize prefix with real words' activations
            if reparam:
                logger.info("prefix_init_by_real_act is only implemented without reparameterization; "
                            "falling back to random init.")
            # For now we only support init_by_real_act for OPT/LLama where the
            # generation API returns past_key_values. For other model types
            # (including our split GPT-2), we silently fall back to random init.
            if mtype in ["opt", "llama"]:
                ...
                logger.info(
                    f"prefix_init_by_real_act is not implemented for model_type={mtype}; "
                    "using random prefix initialization instead."
                )
        ...
        # Replace the old prepare_inputs_for_generation function if the model
        # implements generation. For custom models that do not use this API,
        # this attribute will simply be unused.
    ...
    def add_prefix(self, module, first, input_embeds=None):
        # Infer device/dtype from any parameter in the attention module instead
        # of assuming a specific projection name like `k_proj`.
        ...
        if self.reparam:
            if first:
                ...
                module.prefix_input_embeds = nn.Parameter(
                    torch.randn(self.num_prefix, self.embed_dim, device=device, dtype=dtype),
                    requires_grad=True,
                )
            else:
            ...
            module.prefix_keys = nn.Parameter(
                torch.randn(self.num_prefix, self.hidden_dim, device=device, dtype=dtype),
                requires_grad=True,
            )
