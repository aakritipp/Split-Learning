...
def get_lora_state_dict(model: nn.Module,
    ...
    """
    Return a CPU state dict of LoRA parameters only (keys are full qualified names).
    Optionally filter by OPT decoder layer index via `layer_range`.
    """
    state = {}
    for name, param in model.named_parameters():
        if ("lora_A" in name) or ("lora_B" in name):
            if layer_range is not None:
                li = _layer_index_from_qname(name)
                if li is None or not (layer_range[0] <= li <= layer_range[1]):
                    continue
...
def load_lora_state_dict(model: nn.Module, state: dict) -> int:
    """
    Load LoRA parameters from a dict produced by `get_lora_state_dict` into `model`.
    Only parameters that exist and match in shape are loaded. Returns number of tensors loaded.
    """
    name_to_param = dict(model.named_parameters())
    loaded = 0
    for name, tensor in state.items():
        target = name_to_param.get(name, None)
        if target is None:
            continue
        if target.data.shape != tensor.shape:
            continue
